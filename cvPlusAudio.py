...
# A helper class to smooth out rapid changes in control parameters.
class SmoothValue:
...
# Create a table to capture a short snippet (0.1 sec) of the audio.
table = NewTable(length=0.1)
rec = TableRec(sf, table, fadetime=0.01).play()
...
while True:
    ...
    # If exactly two hands are detected, process landmarks.
    if results.multi_hand_landmarks and len(results.multi_hand_landmarks) == 2:
        ...
        # Compute midpoints of the thumb-index lines.
        ...
        # Use distances between points to control audio parameters.
    ...
    # Get the latest audio samples from the table.
    samples = table.getBuffer()
    # Compute the FFT (only positive frequencies since the signal is real).
    fft_result = np.fft.rfft(samples)
    fft_magnitude = np.abs(fft_result)
    # Normalize the FFT magnitudes for visualization.
    max_val = np.max(fft_magnitude) if np.max(fft_magnitude) != 0 else 1
    fft_magnitude = fft_magnitude / max_val

    # Draw a simple bar graph at the bottom of the image.
    num_bins = len(fft_magnitude)
    bin_width = image.shape[1] / num_bins
    for i, magnitude in enumerate(fft_magnitude):
        x = int(i * bin_width)
        bar_height = int(magnitude * 100)  # Adjust the scaling factor as needed.
        cv2.rectangle(image,
                      (x, image.shape[0]),
